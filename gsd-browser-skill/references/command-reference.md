...
`act-instruction` plans against live DOM affordances and executes existing primitives such as click, type, select-option, set-checked, drag, scroll, reveal-and-click discovery, simple repeated-row choices, DOM-visible counts, and short bounded sequences. Use it for short instructions like "enter 'Alice' into Name and click Save", "choose California from State", "check Red, Green, and Blue", "expand sections to find and click the link", or "buy the shortest duration". Use `--dry-run` to inspect the selected plan, `--scope` to constrain repeated controls to a form/dialog/panel, `--min-confidence` to block weak matches, and `--max-steps` to cap compound actions.
