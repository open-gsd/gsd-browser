...
const DEFAULT_PACKAGE_NAME = '@opengsd/gsd-browser';
...
function loadPackageName(repoRoot = path.resolve(__dirname, '..', '..')) {
  for (const relativePath of ['package.json', 'npm/package.json']) {
    const pkgPath = path.join(repoRoot, relativePath);
    if (!fs.existsSync(pkgPath)) continue;
    const pkgJson = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
    if (pkgJson.name && pkgJson.name !== '@gsd-build/gsd-browser') return pkgJson.name;
  }
...
function collectSections(body) {
  ...
  for (const line of normalizeNewlines(body).split('\n')) {
    ...
    let section = current;
    if (section === "What's Changed") section = classifyTitle(trimmed);
...
function extractInstallCommand(body, packageName, release) {
  ...
  for (const line of lines) {
    ...
    if (inFence && /^npm\s+(?:i|install)\s+/.test(trimmed)) return trimmed;
  }
...
function buildDiscordReleasePayload({ release, packageName = loadPackageName(), maxContent = DEFAULT_MAX_CONTENT }) {
  ...
  if (joinedLength(lines, footer) > maxContent) {
    while (lines.length > baseLines.length && joinedLength(lines, footer) > maxContent) lines.pop();
  }
...
function appendSection(lines, section, bullets, limit, footer, maxContent) {
  ...
  if (remaining > 0) {
    ...
    if (joinedLength([...lines, ...group, moreLine], footer) <= maxContent) group.push(moreLine);
  }
...
function appendLineGroup(lines, group, footer, maxContent) {
  if (joinedLength([...lines, ...group], footer) <= maxContent) lines.push(...group);
}
...
async function postWebhook(webhookUrl, payload) {
  ...
  if (!response.ok) throw new ExitError(1, `Discord webhook failed with ${response.status}: ${await response.text()}`);
}
...
function requireValue(args, flag) {
  ...
  if (!value || value.startsWith('-')) throw new ExitError(2, `error: ${flag} requires a value`);
  return value;
}

async function main() {
  const opts = parseArgs(process.argv.slice(2));
  const release = opts.stdin ? JSON.parse(fs.readFileSync('/dev/stdin', 'utf8')) : fetchRelease(opts);
