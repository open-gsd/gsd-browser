...
| Have a short natural-language action   | `browser_act_instruction` with `dry_run`; handles concise click/fill/select/check, typed controls like number/spinner, range/slider, date/time, color, reveal/count/repeated-row instructions; add `scope`, `min_confidence`, or `max_steps` on ambiguous pages |
